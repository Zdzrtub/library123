...
<body>
    <div class="quan">
        ...
        <form action="rx.php" method="post" enctype="multipart/form-data">
...
<?php 
    ...
    if(isset($_POST['login'])){
		...
		if($usertype=='tian'){
			$sql="INSERT INTO rxtushu (图书名称,作者,图书封面) VALUES ('$tsmc','$zz','$tsmc.png')";
                $res=mysqli_query($con,$sql);
                echo "<script>alert('热销图书登记完成!')</script>";
			}
        if($usertype=='shan'){
			$sql="DELETE from rxtushu WHERE 图书名称='$tsmc' ";
                $res=mysqli_query($con,$sql);
                echo '<script>alert("删除图书完成完成！")</script>';
			}
        } 
        ?>
        <?php
            if(isset($_FILES['upload']))
            {
                ...
                $save='./rxtushu/'.$_FILES['upload']['name'];
